...
{ Int i,							/* idx into the successor array */
	  follow,						/* ifFollow						*/
      loopType, 					/* Type of loop, if any 		*/ 
  	  nodeType;						/* Type of node 				*/
  llIcode opcode;					/* Opcode of last instruction 	*/
		...
		if (nodeType == TWO_BRANCH)		/* if-then[-else] */
		{
		  ...
		  if (pBB->ifFollow != MAX)		/* there is a follow */
		  {
			...
			if (succ->traversed != DFS_ALPHA)	/* not visited */
			{
				...
				{
					l = writeJcond (
						pProc->Icode.icode[pBB->start + pBB->length -1].ic.hl, 
						pProc, numLoc);
...
{	PPROC	pProc;			/* Pointer to procedure */
 	char   *outName, *ext;
